...
public class StackTestCases extends TestCase {
	...
	public void testPushElmToNonFullStack() {
	    ...
	    assertEquals(testElement2, s2.top());
	    //assertEquals(testElement1, s2.top()); //uncomment to check if the stack was correct this will return error
	}
	...
	public void testSameObjectOnTop() {
		...
		//assertEquals(testElement5.getClass(), s3.top().getClass()); //uncomment to check if the stack was correct this will return error
	...
	public void testLIFO() {
		...
		assertEquals(testElement6, s4.pop()); //Check that pop element must be latest element that was push to stack
		assertEquals(s4.top(), s4.pop()); //Check that pop element must be the same as current top element before pop
		assertEquals(testElement1, s4.top()); //Check that if we pop 2 element the first push element must be remain in stack
	...
	public void testLimitStack() {
		Stack s5 = new Stack(3); //Create stack that can contain 3 elements
		Stack s6 = new Stack(8); //Create stack that can contain 8 elements
		
		assertEquals(3, s5.getSize()); //Check that s5 size must be 3
		...
		/*Check that if try to add more element nothing will happened*/
		try {
	        s5.pushStack(testElement1);
	    } catch (Exception e) {
	        fail(e.getMessage());
	    }
		assertEquals(testElement6, s5.top()); //Check that top element should be last element that input before the stack was full
	}
}
